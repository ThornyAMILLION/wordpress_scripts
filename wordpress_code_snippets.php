<?php
            ...
            function check_quantity_of_cart_items() {
                ...
                if (products.length > 0) {
                    ...
                    for (let i of products) {
                        ...
                        if (!displayName.startsWith('8100')) {
    ...
    // Wordpress snippets - Show all products shortcode
                            ...
                            if (typeof response.auto_accept != 'undefined') {
                                if (response.auto_accept == true) {
                                    if (typeof wps_rma_refund_auto_accept == 'function') {
                                        console.log(response.auto_accept)
                                        wps_rma_refund_auto_accept(data.orderid);
                                    }
                                }
                            }
                        }).fail(function(error) {
    ...
    // Wordpress Snippet - List return items shortcode
    add_shortcode('list_returned_items', 'get_all_returned_items');
    function get_all_returned_items() {
        // GET CURRENT USER
        $current_user = wp_get_current_user();
        if (0 == $current_user->ID) {
            return;
        ...
        $customer_orders = get_posts(array(
            ...
            'meta_value'  => $current_user->ID,
            'post_type'   => wc_get_order_types(),
            'post_status' => array("wc-return-requested", "wc-return-approved"),
        ));
        
        $products = [];
        foreach ($customer_orders as $customer_order) {
            $order_obj = wc_get_order($customer_order->ID);
            $item_type = apply_filters('woocommerce_admin_order_item_types', 'line_item');
            $line_items = $order_obj->get_items($item_type);
            $return_datas = get_post_meta($customer_order->ID, 'wps_rma_return_product', true);
            
            $ref_meth = get_option($customer_order->ID . 'wps_rma_refund_method');
                ...
                foreach ($line_items as $item_id => $item) {
                    foreach ($return_products as $returnkey => $return_product) {
                        if ($return_product['item_id'] == $item_id) {
                            ...
                            array_push($products, ['id' => $item->get_variation_id(), 'user' => $current_user->display_name, 'product' => $product->get_name(), 'qty' => $return_product['qty'], 'price' => $return_product['price'], 'order_id' => $customer_order->ID, 'date' => $key, 'status' => $return_data['status']]);
                        }
                    }
                }
            }
        ...
        $html = '<div><table><thead><tr><th>Id</th><th>Customer</th><th>Product</th><th>Order Id</th><th>Quantity</th><th>Price</th><th></th></tr></thead>';
        
        foreach($products as $product) {
            $html .= '<tr><td>' . $product['id'] . '</td><td>' . $product['user'] . '</td><td>' . $product['product'] . '</td><td>' . $product['order_id'] . '</td><td>' . $product['qty'] . '</td><td>$' . number_format($product['price'], 2) . '</td><td>';
            
            if ($product['status'] === 'pending') {
                $html .= '<button class="wps_rma_accept_return" type="button" data=orderid="' . $product['order_id'] . '" data-date="' . $product['date'] . '">Accept</button><button type="button">Cancel</button>';
            } else {
                $html .= $product['status'];
            }
    ...
    // Wordpress snippets - Accept Refund
    add_action('wp_footer', function() {
        ?>
        <script>
            jQuery(document).ready(function() {
                $(document).on('click', '.wps_rma_accept_return', function() {
                    ...
                    let product_info = row.children();
                    let data = {
                        action: 'return_req_approve',
                        orderId: orderId,
                        productid: product_info[0].innerText 
                    };
